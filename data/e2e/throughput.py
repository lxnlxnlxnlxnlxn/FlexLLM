...
def main() -> None:
    ...
    fig.suptitle("Inference Throughput", fontsize=30, fontweight="bold", y=0.90)
    handles, labels = axes_list[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="upper center", ncol=5, 
               fontsize=25, frameon=False, bbox_to_anchor=(0.5, 0.88))
    plt.savefig(".\\data\\e2e\\throughput.png", dpi=300, bbox_inches='tight')
...
def fit(ax: Any, data: Dict[str, List[float]], model: str, dataset: str) -> None:
    ...
    ax.set_ylabel("Throughput(seq/s)", fontsize=25, fontweight="bold")
    ax.set_ylim(0, 1.1)
...
def read(model: str, dataset: str) -> Dict[str, List[float]]:
    ...
    df = pd.read_excel(data_path, sheet_name="throughput", header=None, engine="openpyxl")
    raw = df.values
    return {
